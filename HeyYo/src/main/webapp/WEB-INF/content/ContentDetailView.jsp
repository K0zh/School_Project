...
<body>
	...
	<div id="wrapper">
		...
		<div>
			...
			상세보기 페이지 입니다.<br> NUM : ${content.num }<br> 아이디 :
			${content.id } <br> 분류 : ${content.type } <br> 시간 :
			${content.s_day } <br> 작성일 : ${content.c_date } <br> 재능 :
			${content.talent } <br> 제목 : ${content.subject } <br> 본문 내용
			: ${content.content } <br> 주소 : ${content.address } <br> 세션
			: ${sessionScope.id} 테스트4443
		</div>
		<br> <input type="button" value="신청하기" id="requestBtn"
			onclick="matching()">
		<div class="form-inline" id="requestDiv">

			<form action="matching.con" method="post">
				<input type="hidden" name="m_num" value="${content.num }"> <input
					type="hidden" name="m_name" value="${sessionScope.name }">
		...
		<table class="table table-bordered table-hover">
			...
			<tbody>
				<c:forEach items="${matchingList}" var="matching" varStatus="status">
					<tr>
						<td>${status.count }</td>
						<td><a href="user_detail.mb?id=${matching.M_ID}">${matching.M_NAME}</a>
						</td>
						<td><a href="user_detail.mb?id=${matching.M_ID}">${matching.INTRODUCE}</a>
						</td>
						<td>
